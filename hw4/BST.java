...
public class BST<E> extends BinaryTree<E> {
	...
	public BST(Comparator<E> c) {
		c1 = c;// assign the comparator value
	}
	...
	public boolean contains(E e) {
		...
		while (current != null) {
			if (c1.compare(e, current.getData()) < 0) {
				current = current.getLeftChild();
			} else if (c1.compare(e, current.getData()) > 0) {
	...
	private BinaryNode<E> _findNode(BinaryNode<E> node, E e) {
		...
		// find the node
		else if (c1.compare(e, node.getData()) < 0) {
	...
	private BinaryNode<E> _insert(BinaryNode<E> node, E e) {
		...
		if (node == null) {
			...
			if (c1.compare(e, node.getData()) < 0)
