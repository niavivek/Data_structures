...
public class Kruskal<E> extends Graph<E>
	...
	public ArrayList<Edge<E>> applyKruskal()
		...
		while (!edgeHeap.isEmpty() && vertexSets.size() > 1)
			...
			for (fIter = vertexSets.iterator(), numVertsFound = 0; fIter
					.hasNext() && (numVertsFound < 2);)
				{
				vertSet = fIter.next();
	...
	private boolean buildEdgeHeap()
		...
		for (vertIter = vertsInGraph.entrySet().iterator(); vertIter.hasNext();)
			...
			for (edgeIter = src.adjList.entrySet().iterator(); edgeIter
					.hasNext();)
				{
	...
	public void printMSTKruskal()
		{
		System.out
				.println("The minimum spanning tree according to Kruskal's algorithm is");
		Iterator iter1 = applyKruskal().iterator();
		double length = 0;
		while (iter1.hasNext())
			{
			Edge<E> values = (Edge<E>) iter1.next();
